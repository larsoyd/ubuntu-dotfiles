...
Description: This patch turns the METRO indicators under the icons from rectangles that awkwardly bump into each other in ugly ways into slightly inset rounded capsules. It also centers them under the icon. It uses existing dot size + color settings, and only changes geometry. This looks much more pleasing to the eye in my opinion.
